...
export default function BackgroundChecksPage() {
  ...
  async function handleSync() {
    ...
    try {
      ...
      if (res.ok) { showToast(`Synced ${data.synced} contracts`); loadData() }
  ...
  async function openChat(rec: BgCheck) {
    ...
    try {
      ...
      if (conversations.length === 0) {
        setChat(prev => prev ? { ...prev, loading: false } : null)
        return
      }
      ...
      const messages: Message[] = (msgData.messages?.messages || msgData.messages || []).sort(
        (a: Message, b: Message) => new Date(a.dateAdded).getTime() - new Date(b.dateAdded).getTime()
      )
  ...
  async function sendMessage() {
    ...
    try {
      const res = await fetch("/api/ghl/send-message", {
        ...
        body: JSON.stringify({ contactId: chat.contactId, message: newMsg.trim(), type: "SMS" }),
      })
      const data = await res.json()
      if (res.ok && !data.error) {
        ...
        setChat(prev => prev ? { ...prev, sending: false } : null)
      }
    } catch (err: unknown) {
      showToast("Send failed: " + (err instanceof Error ? err.message : "Unknown"))
      setChat(prev => prev ? { ...prev, sending: false } : null)
    }
  }
  ...
  const filtered = filter === "all" ? searched :
    ...
    filter === "done" ? searched.filter(r => !!(r.shirt_given_at && r.badge_printed_at)) : searched
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      {/* Main content */}
      <div style={{ flex: 1, padding: "16px 20px", maxWidth: 900, margin: "0 auto", transition: "margin-right .3s", marginRight: chat ? 400 : 0 }}>
        {toast && <div style={{ position: "fixed", top: 20, right: chat ? 420 : 20, background: "#1a2f6e", color: "white", padding: "12px 20px", borderRadius: 8, zIndex: 9999, fontSize: 14, boxShadow: "0 4px 12px rgba(0,0,0,.2)" }}>{toast}</div>}

        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#1a2f6e", margin: 0 }}>HD Background Checks</h1>
          <button onClick={handleSync} disabled={syncing} style={{ padding: "8px 16px", borderRadius: 8, border: "2px solid #1a2f6e", background: syncing ? "#1a2f6e" : "white", color: syncing ? "white" : "#1a2f6e", fontWeight: 600, fontSize: 13, cursor: syncing ? "wait" : "pointer" }}>
          ...
          return (
            <div key={rec.id} style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16, marginBottom: 10, background: isDone ? "#f9fafb" : "white", opacity: isDone ? 0.75 : 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                <div onClick={() => openChat(rec)} style={{ fontWeight: 700, fontSize: 16, color: "#1a2f6e", cursor: "pointer", textDecoration: "underline", textDecorationColor: "transparent", transition: "text-decoration-color .2s" }} onMouseEnter={(e) => { e.currentTarget.style.textDecorationColor = "#1a2f6e" }} onMouseLeave={(e) => { e.currentTarget.style.textDecorationColor = "transparent" }}>
            ...
            {chat.loading ? (
              ...
              <div key={msg.id} style={{ display: "flex", justifyContent: msg.direction === "outbound" ? "flex-end" : "flex-start", marginBottom: 8 }}>
                <div style={{
                  ...
                  background: msg.direction === "outbound" ? "#1a2f6e" : "white",
          ...
          <div style={{ padding: 12, borderTop: "1px solid #e5e7eb", background: "white" }}>
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                style={{ flex: 1, padding: "10px 14px", border: "2px solid #e5e7eb", borderRadius: 10, fontSize: 14, outline: "none", boxSizing: "border-box" }}
                onFocus={(e) => { e.currentTarget.style.borderColor = "#1a2f6e" }}
                onBlur={(e) => { e.currentTarget.style.borderColor = "#e5e7eb" }}
              />
              <button onClick={sendMessage} disabled={chat.sending || !newMsg.trim()}
                style={{ padding: "10px 20px", borderRadius: 10, border: "none", background: chat.sending || !newMsg.trim() ? "#9ca3af" : "#1a2f6e", color: "white", fontWeight: 700, fontSize: 14, cursor: chat.sending ? "wait" : "pointer" }}>
                {chat.sending ? "..." : "Send"}
              </button>
            </div>
          </div>

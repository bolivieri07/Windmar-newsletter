...
type Totals = { total: number; completed: number; sent: number; viewed: number; draft: number }
...
export default function BackgroundChecksPage() {
  ...
  function updateBoth(updater: (prev: MergedRecord[]) => MergedRecord[]) {
    setRecords(updater)
  ...
  async function mergeWithLocal(docs: GHLDocument[]): Promise<MergedRecord[]> {
    ...
    return docs.map(doc => {
      ...
      return {
        documentId: doc.documentId, contactId: recipient?.id || "", repName: recipient?.contactName || "Unknown",
        repEmail: recipient?.email || "", docStatus: doc.status, createdAt: doc.createdAt, updatedAt: doc.updatedAt,
        signedDate: recipient?.signedDate || null, hrStatus: local?.hr_status || "pending",
        approvedAt: local?.approved_at || null, shirtGivenAt: local?.shirt_given_at || null,
        badgePrintedAt: local?.badge_printed_at || null, localId: local?.id || null, files: null, loadingFiles: false,
      }
    })
  }
  ...
  const fetchPage = useCallback(async (currentSkip: number, append: boolean) => {
    ...
    try {
      ...
      if (append) {
        setRecords(prev => { const ids = new Set(prev.map(r => r.documentId)); return [...prev, ...newMerged.filter(r => !ids.has(r.documentId))] })
      } else { setRecords(newMerged) }
  ...
  useEffect(() => {
    ...
    const localMatches = records.filter(r => r.repName.toLowerCase().includes(lower) || r.repEmail.toLowerCase().includes(lower))
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        const data = await res.json()
        const bgDocs = (data.documents || []).filter((d: GHLDocument) => d.name === BG_CHECK_DOC_NAME && d.status !== "draft")
        if (bgDocs.length > 0) {
          const merged = await mergeWithLocal(bgDocs)
          const allIds = new Set<string>()
          const combined: MergedRecord[] = []
          for (const r of [...localMatches, ...merged]) { if (!allIds.has(r.documentId)) { allIds.add(r.documentId); combined.push(r) } }
          combined.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
          setSearchResults(combined)
        } else if (localMatches.length === 0) { setSearchResults([]) }
      } catch { if (localMatches.length === 0) setSearchResults([]) }
      setSearching(false)
    }, 500)
    return () => { if (searchTimer.current) clearTimeout(searchTimer.current) }
  }, [search, records, supabase])

  function handleLoadMore() { const next = skip + PAGE_SIZE; setSkip(next); fetchPage(next, true) }
  function handleRefresh() { setSkip(0); setHasMore(true); setRecords([]); setSearch(""); setSearchResults(null); loadTotals(); fetchPage(0, false) }
  ...
  async function ensureLocalRecord(rec: MergedRecord): Promise<string> {
    ...
    const { data, error } = await supabase.from("background_checks").insert({
      ghl_document_id: rec.documentId, rep_name: rec.repName, rep_email: rec.repEmail,
      document_status: rec.docStatus, hr_status: "pending", fulfillment_status: "pending",
    }).select().single()
    if (error) { showToast("Error: " + error.message); return "" }
    updateBoth(prev => prev.map(r => r.documentId === rec.documentId ? { ...r, localId: data.id } : r))
    return data.id
  }
  ...
  async function updateHRStatus(rec: MergedRecord, status: string) {
    ...
    updateBoth(prev => prev.map(r => r.documentId === rec.documentId ? { ...r, hrStatus: status, approvedAt: status === "approved" ? new Date().toISOString() : r.approvedAt } : r))
  ...
  async function updateFulfillment(rec: MergedRecord, field: string) {
    ...
    await supabase.from("background_checks").update(updates).eq("id", localId)
    updateBoth(prev => prev.map(r => r.documentId === rec.documentId ? { ...r, shirtGivenAt: field === "shirt" ? now : r.shirtGivenAt, badgePrintedAt: field === "badge" ? now : r.badgePrintedAt } : r))
  ...
  async function loadContactFiles(rec: MergedRecord) {
    ...
    updateBoth(prev => prev.map(r => r.documentId === rec.documentId ? { ...r, loadingFiles: true } : r))
    try {
      ...
      updateBoth(prev => prev.map(r => r.documentId === rec.documentId ? { ...r, files, loadingFiles: false } : r))
    } catch {
      updateBoth(prev => prev.map(r => r.documentId === rec.documentId ? { ...r, files: {}, loadingFiles: false } : r))
  ...
  async function handleResendContract(rec: MergedRecord) {
    ...
    try {
      const res = await fetch("/api/ghl/resend-document", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ documentId: rec.documentId, locationId: LOCATION_ID }) })
      const data = await res.json()
  ...
  const filtered = filter === "all" ? activeRecords :
    ...
    filter === "done" ? activeRecords.filter(r => !!(r.shirtGivenAt && r.badgePrintedAt)) : activeRecords
  ...
  return (
    <div style={{ padding: "16px 20px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
        {[
          ...
          <button key={s.key} onClick={() => setFilter(s.key)} style={{ flex: "1 1 100px", padding: "10px 8px", border: filter === s.key ? `2px solid ${s.color}` : "2px solid #e5e7eb", borderRadius: 10, background: filter === s.key ? s.color : "white", color: filter === s.key ? "white" : s.color, cursor: "pointer", textAlign: "center", fontWeight: 600, fontSize: 13, transition: "all .2s" }}>
            <div style={{ fontSize: 22, fontWeight: 700 }}>{totalsLoading && (s.key === "all" || s.key === "sent" || s.key === "completed") ? "..." : s.value}</div>
            {s.label}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", color: "#9ca3af", padding: 40, fontSize: 15 }}>{searching ? "Searching GHL..." : search ? `No contracts found for "${search}"` : "No records in this category"}</div>
      ) : filtered.map(rec => {
          ...
          <div key={rec.documentId} style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16, marginBottom: 10, background: isDone ? "#f9fafb" : "white", opacity: isDone ? 0.75 : 1 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
              ...
              {(rec.docStatus === "sent" || rec.docStatus === "viewed") && (
                <button onClick={() => handleResendContract(rec)} disabled={resendingId === rec.documentId} style={{ padding: "6px 14px", borderRadius: 8, border: "2px solid #b45309", background: "white", color: "#b45309", fontWeight: 600, fontSize: 13, cursor: resendingId === rec.documentId ? "wait" : "pointer", opacity: resendingId === rec.documentId ? 0.6 : 1 }}>
      ...
      {!search && (<div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        {hasMore && (<button onClick={handleLoadMore} disabled={loadingMore} style={{ flex: 1, padding: "10px 20px", borderRadius: 8, border: "none", background: "#1a2f6e", color: "white", fontWeight: 600, cursor: loadingMore ? "wait" : "pointer", opacity: loadingMore ? 0.7 : 1 }}>{loadingMore ? "Loading more..." : `Load More (showing ${records.length})`}</button>)}

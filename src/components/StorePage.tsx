            ...
            <div key={p.id} onClick={() => setSelected(p)}
              ...
              <div style={{ height: 200, background: "#f3f4f6", overflow: "hidden" }}>
                {p.image ? (
                  ...
                  <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg,#0f1d47,#1a2f6e)" }}>
                    <span style={{ fontSize: "3rem" }}>👕</span>
                  </div>
                )}
              </div>
              <div style={{ padding: "1rem" }}>
                <div style={{ fontWeight: 800, color: "#1a2f6e", fontSize: "1rem", marginBottom: "0.25rem" }}>{p.title}</div>
                {p.description && (
                  <div style={{ fontSize: "0.82rem", color: "#6b7280", marginBottom: "0.75rem", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" } as any}>
                    {p.description}
                  </div>
                )}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ fontSize: "1.1rem", fontWeight: 800, color: "#1a2f6e" }}>${p.price.toFixed(2)}</div>
                  <button style={{ padding: "0.45rem 1rem", background: "#f89b24", color: "white", border: "none", borderRadius: 8, fontSize: "0.82rem", fontWeight: 700, cursor: "pointer", fontFamily: "Barlow,system-ui,sans-serif" }}>
                    Buy Now
                  </button>
                </div>
                {p.variants.length > 1 && (
                  <div style={{ fontSize: "0.72rem", color: "#9ca3af", marginTop: "0.4rem" }}>{p.variants.length} options available</div>
                )}
              </div>
            </div>
          ))}
